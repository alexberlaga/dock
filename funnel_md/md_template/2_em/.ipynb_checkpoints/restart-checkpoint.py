...
def restart(top_file,
    ...
    helix_backbone_idxs = np.array([md_top.top.select('resid 0 to 30 and (name CA or name NL or name CLP)'), md_top.top.select('resid 31 to 61 and (name CA or name NL or name CLP)'), md_top.top.select('resid 62 to 92 and (name CA or name NL or name CLP)')])
    ...
    start_step = time_data[-1, 0] % save_restart_freq
